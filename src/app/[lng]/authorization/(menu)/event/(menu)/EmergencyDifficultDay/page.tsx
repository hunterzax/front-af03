...
const ClientPage: React.FC<ClientProps> = (props) => {
    ...
    const handleFieldSearch = async () => {
        ...
        // 3. DOC3.9 & DOC4 : TSO > กด Gen Doc มาจาก Balance ตอนสถานะเอกสารยังเป็น generated Shipper ต้องยังไม่เห็นเอกสารอันนี้ จนกว่า TSO จะเปลี่ยนสถานะเป็น opend Shipper ถึงจะเห็นได้
        // กรอง withZoneName ที่มี document39.event_doc_status.id == 6 หรือ document41.event_doc_status.id == 6 ออกไปเลย
        const filteredGenerateOutBro = (result_filter_type ?? []).filter((it: any) => {
            const is39Gen = it?.document39?.event_doc_status?.id === 6;
            const is41Gen = Array.isArray(it?.document41) ? it.document41.some((d: any) => d?.event_doc_status?.id === 6) : it?.document41?.event_doc_status?.id === 6; // กันเหนียวเผื่อเป็น obj

            return !is39Gen && !is41Gen;
        });

        if (userDT?.account_manage?.[0]?.user_type_id == 3 || userDT?.account_manage?.[0]?.user_type_id == 4) {
            setDataTable(filteredGenerateOutBro)
            setFilteredDataTable(filteredGenerateOutBro)
        } else {
    ...
    const handleSearch = (query: string) => {
        ...
        const filtered = dataTable?.filter(
            (item: any) => {
                ...
                const doc39StatusMatch = itemdoc39.length > 0 && itemdoc39.some((d: any) => norm(d?.event_doc_status?.name).includes(queryLower));

                //===================================================================================
                const itemdoc4: any = item?.document41;
                const datashiper4: any = getAcknowledgeStatus(itemdoc4);
                const doc4StatusMatch = itemdoc4.length > 0 && itemdoc4.some((d: any) => norm(d?.event_doc_status?.name).includes(queryLower));
                ...
                return (
                    ...
                    (itemdoc39?.length > 0 && datashiper39?.replace(/\s+/g, '').toLowerCase().trim()?.includes(queryLower)) ||
                    (itemdoc4?.length > 0 && datashiper4?.replace(/\s+/g, '').toLowerCase().trim()?.includes(queryLower)) ||
    ...
    const fetchData = async (offset?: any, limit?: any) => {
        try {
            ...
            if ((status_from_somewhere_else || start_date_from_somewhere_else || end_date_from_somewhere_else) && !useParams) {
                ...
                res_API = await getService(`/master/event/emer?eventCode=&eventDateFrom=${start_date}&eventDateTo=${end_date}&EventStatus=${eventStatAPI}&event_doc_emer_type_id=&offset=${offset}&limit=${limit}`);
                setuseParams(true);
            } else {
                res_API = await getService(`/master/event/emer?eventCode=&eventDateFrom=${eventDateFrom}&eventDateTo=${eventDateTo}&EventStatus=&event_doc_emer_type_id=&offset=${offset}&limit=${limit}`);
            }

            // แปลงพวก onshore east, west ให้เป็นชื่อโซน
            const withZoneName = res_API?.data?.map((item: any) => {
            ...
            // 3. DOC3.9 & DOC4 : TSO > กด Gen Doc มาจาก Balance ตอนสถานะเอกสารยังเป็น generated Shipper ต้องยังไม่เห็นเอกสารอันนี้ จนกว่า TSO จะเปลี่ยนสถานะเป็น opend Shipper ถึงจะเห็นได้
            // กรอง withZoneName ที่มี document39.event_doc_status.id == 6 หรือ document41.event_doc_status.id == 6 ออกไปเลย
            const filteredGenerateOutBro = (withZoneName ?? []).filter((it: any) => {
                const is39Gen = it?.document39 ? (it?.document39 && it?.document39?.event_doc_status?.id === 6) : true; 
                const is41Gen = it?.document41?.length > 0 && it.document41.some((d: any) => d?.event_doc_status?.id === 6);
                // return !is39Gen && !is41Gen;
                return !is39Gen || !is41Gen;
            ...
            // console.log('filteredGenerateOutBro : ', filteredGenerateOutBro);
            // console.log('withZoneName : ', withZoneName);
            if (userDT?.account_manage?.[0]?.user_type_id == 3 || userDT?.account_manage?.[0]?.user_type_id == 4) {
                setDataTable(filteredGenerateOutBro)
                setFilteredDataTable(filteredGenerateOutBro)
            } else {
        ...
        switch (WhichOpenDocument) {
            case 'document_39':
                if (modeOpenDocument == 'create') {
                    ...
                    if (isTSO) {
                        ...
                        const status = res_tso_edit?.response?.data?.statusCode ?? res_tso_edit?.response?.data?.status ?? res_tso_edit?.status ?? res_tso_edit?.statusCode ?? res_tso_edit?.code ?? res_tso_edit?.response?.status;
                        const errorMsg = res_tso_edit?.response?.data?.error ?? res_tso_edit?.data?.error ?? res_tso_edit?.response?.error ?? res_tso_edit?.error;
            ...
            case 'document_4':
                ...
                if (modeOpenDocument == 'create') {
                    ...
                    if (isTSO) {
                        ...
                        const status = res_edit_shipper?.response?.data?.statusCode ?? res_edit_shipper?.response?.data?.status ?? res_edit_shipper?.status ?? res_edit_shipper?.statusCode ?? res_edit_shipper?.code ?? res_edit_shipper?.response?.status;
                ...
                if (modeOpenDocument == 'create') {
                    ...
                    if (isTSO) {
                        ...
                        const payload_tso_edit = {
                            ...
                            // "cc_email": item?.cc_email
                            ...item,
                            doc_5_input_event_date: item?.doc_5_input_event_date ? dayjs(item?.doc_5_input_event_date).toDate() : null
                        }
                        console.log('item : ', item);
                        ...
                        const status = res_edit_shipper?.response?.data?.statusCode ?? res_edit_shipper?.response?.data?.status ?? res_edit_shipper?.status ?? res_edit_shipper?.statusCode ?? res_edit_shipper?.code ?? res_edit_shipper?.response?.status;
                        const errorMsg = res_edit_shipper?.response?.data?.error ?? res_edit_shipper?.data?.error ?? res_edit_shipper?.response?.error ?? res_edit_shipper?.error;
            ...
            case 'document_6':
                if (modeOpenDocument == 'create') {
                    ...
                    if (isTSO) {
                        // TSO edit DOC 6 — send gas_shipper exactly as assembled by the form (no extra filtering)
                        const payload_tso_edit = {
    ...
    const initialColumns: any = [
        ...
        { key: 'document_39', label: 'Emergency Doc.1', visible: true },
        { key: 'document_4', label: 'Emergency Doc.2', visible: true },
        { key: 'document_5', label: 'Emergency Doc.3', visible: true },
                                ...
                                (userDT?.account_manage?.[0]?.user_type_id !== 3 && userDT?.account_manage?.[0]?.user_type_id !== 4) && <>
                                    <div className="flex flex-wrap gap-2 justify-end">
                                        <BtnGeneral
                                            ...
                                            textRender={"Incident Report (Emergency Doc.1)"}
                                            iconNoRender={true}
                                            bgcolor={"#00ADEF"}
                                            generalFunc={() => openDocument('document_39', 'create')}
                                            disable={false}
                                            customWidthSpecific={280}
                                    ...
                                    <div className="flex flex-wrap gap-2 justify-end">
                                        <BtnGeneral
                                            textRender={"Instruction Report (Emergency Doc.2)"}
                                            iconNoRender={true}
                                            bgcolor={"#00ADEF"}
                                    ...
                                    <div className="flex flex-wrap gap-2 justify-end">
                                        <BtnGeneral
                                            textRender={"Recovery Report (Emergency Doc.3)"}
                                            iconNoRender={true}
                                            bgcolor={"#00ADEF"}
                                    ...
                                    <div className="flex flex-wrap gap-2 justify-end">
                                        <BtnGeneral
                                            textRender={"Adjustment Report (Doc.6)"}
                                            iconNoRender={true}
                                            bgcolor={"#00ADEF"}
